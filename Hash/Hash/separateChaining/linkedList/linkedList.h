...
/*  ************* !!! 链表中头结点，操作时容易引起错误 !!! *****************
/*
/*  List和pNode都是指针，上述函数操作都是传递指针，在传递时，编译器新建指针副本
/*  （等效于List *newList = L），在函数内实际是用newList进行操作，newList的变
/*  化只能改变L指针指向的内容，不能改变L指针本身的地址，这是在函数中可以用L直接
/*  遍历的原因（遍历后newList的地址和指针内容都发生了变化）
/*  
/************************************************************************/
...
void insert_head (List L, int Value);       //在链表头部插入元素
void insert_tail (List L, int Value);       //在链表尾部插入元素
bool insert (List L, int Value, int index); //在index处插入元素

void delete_head (List L);           //删除头部元素
void delete_tail (List L);           //删除尾部元素
void deleteNode (List L, int index); //删除index处元素
void deleteList (List &L);            //清空链表(包括头部节点)

int search (List L, int Value);      //搜索链表中值为value的元素，返回其index，没有则返回NotInList（如果有多个值为value的元素，则只返回第一个元素的索引值）

pNode getNode(List L, int index);   //返回链表中index处的节点指针

int getHead (List L);                //返回头部元素
int getTail (List L);                //返回尾部元素
int getValve (List L, int index);    //返回index处的元素
